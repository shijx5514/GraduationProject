...
import math
"""
实现了一个简易的人工神经网络，部分实现方式借鉴了一些网上的成熟实现。
"""

class BPNauralNetwork:

    #构造函数
    # @params: sizes 为list类型 ，例如 list [2,3,1]代表着几个三层每层神经元分别为2，3，1的神经网络。
    def _init_(self, sizes):
        self.layers_num = len(sizes);#有几层神经网络
        self.sizes = sizes;
        # 随机产生每层中y个神经元的biase值 （0-1）
    ...
    #前馈
    #前向传输计算每个神经元的值,param a; 输入值， return：计算后每个神经元的值
    def feedforward(selfs, a):
        for b,w in zip(selfs.biases, selfs.weights):
            # 逻辑 sigmoid函数
            a =  selfs.sigmoid(np.dot(w, a) + b)
        return a
